...
不要重复写相同的代码 —— 这是你在作为软件工程师的学习和工作过程中，被灌输的几个重要原则之一。它植根于我们内心，以至于我们对难以容忍代码的冗余，即使只有几行的重复代码也不行。总体来说它是一个好的基本原则，但如果过度遵循它，它就会带来过多的认知负荷，让我们难以处理。

现如今，人们都基于逻辑分离的组件构建软件。这些通常分布在多个代码库中，且代表着不同的服务。在你努力消除重复的过程中，可能会使不相关的组件之间产生紧密耦合。这样一来，后续在一个部分的更改可能会在其他看似不相关的地方产生意想不到的后果。同时也会阻碍在不影响整个系统的情况下替换或修改单个组件的能力。`🤯`

事实上，即使在单个模块软件中也会出现类似的问题。你可能会过早地把通用功能提取出来，这种提取是基于一种自我感觉的，但是从长远角度看可能并不存在的相似性。而这样做可能会引发不必要的抽象，这些抽象会使得软件难以修改或扩展

Rob Pike 曾经说过：

> 自己一点点地进行复制，要比一点点地依赖他人更好。「A little copying is better than a little dependency」

我们不想去重新创造已经很强大的东西，比如一个功能强大的 “轮子” 。以至于我们愿意引入大型的、比较复杂的库，即使只是为了使用其中一些我们自己本来也可以轻松编写出来的小函数。

**你的所有依赖项都是你的代码。** 浏览跟踪 10 个以上层级的堆栈以找出问题（*因为出了岔子*）是一件痛苦的事。

## 与框架紧密耦合(Tight coupling with a framework)

框架中有很多 “魔力”。如果我们过于依赖框架，**我们将迫使所有即将参与的开发人员首先学习这种 “魔力”**。这可能需要几个月的时间。虽然框架可以让我们能够在几天内启动 MVP【最小可行产品】，但长远来看，它们往往会增加不必要的复杂性和认知负荷。

更糟的是，在某些时候，当遇到一个与现有架构不匹配的新需求时，框架可能会在很大程度上对实现这个需求形成阻碍。从这里开始，人们会在当前框架基础上建立一个新的分支并维护一个定制版本。设想一下，一个刚加入的新人，为了能够进行开发，需要承担多少认知负荷（即学习这个定制版本的框架）。`🤯`

**这绝不意味着我们提倡从零开始开发所有东西！**

我们能够采用不受特定框架限制的方式来编写代码。例如业务逻辑不应该被放置在框架之中；相反，它应该通过调用框架所提供的组件来实现业务逻辑。在核心业务逻辑之外搭建一个框架来处理一些通用功能。然后其他开发人员可以像使用一个类库一样来调用框架。这样做能够让新的开发者在一开始就发挥作用，而不必先去了解和框架有关的复杂内容。

## 六角 / 洋葱架构(Hexagonal/Onion architecture)
