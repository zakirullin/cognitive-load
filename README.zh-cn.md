...
<details>
  ...
  比如说，你能想象吗，<code>||</code>在<code>requires ((!P<T> || !Q<T>))</code> 和 <code>requires (!(P<T> || Q<T%gt;))</code>中有不同的含义。第一种情况是约束析取「constraint disjunction」，第二种情况是常见的逻辑或「logical OR」运算符，而且它们的行为是不一致的。<br><br>
