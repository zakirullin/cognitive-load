...
When reading through code, you hold various facts in memory, such as conditional logic, boolean variables values, execution flow, other component's contract, possible effect on the coupled code and so on. The average person can hold roughly [four unrelated facts](https://github.com/zakirullin/cognitive-load/issues/16) in working memory. Once the cognitive load reaches this threshold, a significant effort is required to understand things.
