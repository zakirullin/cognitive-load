...
<details>
  ...
  Like, can you imagine, token `||` has different meaning in <code>requires (!(P<T> || Q<T>))</code> and in <code>requires ((!P<T> || !Q<T>)). The first is the constraint disjunction, the second is good-old logical OR operator, and they behave differently.</code>.<br><br>
